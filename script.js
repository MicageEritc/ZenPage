...
document.addEventListener("DOMContentLoaded", () => {
    ...
    engineIcons.forEach(icon => {
        if (icon.getAttribute("data-engine") === currentEngine) {
            ...
            searchInput.placeholder = searchPlaceholders[currentEngine];
        } else {
...
function initWallpaper() {
    const isBingWallpaper = localStorage.getItem("bingWallpaper") === "true";
    if (isBingWallpaper) {
        setBingBackground();
        document.body.classList.add("bing-wallpaper");
        wallpaperToggle.innerHTML = '<i class="fas fa-palette"></i>';
    }
}

// 切换壁纸
function toggleWallpaper() {
    const isBingWallpaper = document.body.classList.toggle("bing-wallpaper");
    localStorage.setItem("bingWallpaper", isBingWallpaper);
    ...
    if (isBingWallpaper) {
        ...
        // 恢复默认渐变背景 - 修复这里
        document.body.style.backgroundImage = "linear-gradient(135deg, rgba(37, 102, 176, 0.8) 0%, rgba(118, 213, 245, 0.8) 50%, rgba(155, 197, 255, 0.8) 100%)";
        wallpaperToggle.innerHTML = '<i class="fas fa-image"></i>';
    }
}
...
// 设置事件监听器 - 删除第二个重复的setupEventListeners函数
function setupEventListeners() {
    // 搜索引擎切换 - 添加记忆功能
    engineIcons.forEach(icon => {
        icon.addEventListener("click", () => {
